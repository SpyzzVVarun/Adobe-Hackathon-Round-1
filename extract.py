...
from adobe.pdfservices.operation.pdfops.options.extractpdf.extract_element_type import ExtractEleextract.py
# Adobe PDF Extract SDK Imports
from adobe.pdfservices.operation.auth.credentials import Credentials
from adobe.pdfservices.operation.exception.exceptions import ServiceApiException, ServiceUsageException, SdkException
from adobe.pdfservices.operation.execution_context import ExecutionContext
...
def solve(TestDataSet = "InvoicesData/InvoicesData/TestDataSet/", 
        ...
        OutputDir = 'InvoicesData/ReRunTestOutput/'):
...
def extractCustomerDetails(records, text, newline_text, text2path):
    ...
    if(not customer_details_part1.strip() and not customer_details_part2.strip()):
        # In general case of this format, customer details are found in this layout
        customer_details = newline_text.split("PAYMENT")[1].split("Due date")[0]

        # Phone Number Regex Search
        pattern = r'\d{3}-\d{3}-\d{4}'
        match = re.findall(pattern, customer_details)
        if match:
            match = match[0]
            records['Customer__PhoneNumber'] = match
            customer_details = customer_details.split(match)
            customer_details[0] = customer_details[0].replace("\n", " ")
            customer_details[1] = customer_details[1][1:-1].strip().split("\n")

            # Multiple Address Lines
            if(len(customer_details[1]) > 2):
                records['Customer__Address__line1'] = customer_details[1][0]
                records['Customer__Address__line2'] = customer_details[1][1]
                customer_details[1][2] = " ".join(customer_details[1][2:])
                records['Invoice__Description'] = customer_details[1][2]
            
            # Single Address Line
            else:
                records['Customer__Address__line1'] = customer_details[1][0]
                records['Customer__Address__line2'] = ""
                records['Invoice__Description'] = customer_details[1][1]

            # Email Regex Search
            pattern = r'\b\w+[\w.]*\w+@\w+[\w.]*\w+\b'
            match = re.findall(pattern, customer_details[0])[0]
            records['Customer__Name'] = customer_details[0].split(match)[0].strip()
            records['Customer__Email'] = match + customer_details[0].split(match)[1].replace(" ", "")
        
        # In special case of this format, customer details are found in this layout
        else:
            customer_details = newline_text.split("PAYMENT")[1].split("ITEM")[0]

            # Phone Number Regex Search
            number = re.findall(r'\d{3}-\d{3}-\d{4}', customer_details)[0]
            records['Customer__PhoneNumber'] = number

            # Email Regex Search
            email = re.findall(r'\b\w+[\w.]*\w+@\w+[\w.]*\w+\b', customer_details)[0]
            records['Customer__Email'] = email

            # Extracting name based on fuzzy string matching with the extracted email
            name = process.extract(email, customer_details.split("\n"))[1][0]
            records['Customer__Name'] = name

            # Date and Amount Extraction
            date = customer_details.split("Due date: ")[1].split("\n")[0]
            amount = re.findall(r"\$\d+(?:\.\d{1,2})?", customer_details)[0]

            # Strategy to extract Invoice Description and Customer Address in this special formatting case.
            # First, I remove the already found entites in the customer details section. 
            # Next I see that the remaining text lies in the 2 entities remaining. So, I use the path information 
            # to differentiate between the 2.
            found = [number, email, name, "Due date: " + date, amount]
            found = [word.strip() for word in found]
            words = [word.strip() for word in customer_details.split('\n') if word.strip()]
...
def extractInvoiceBillDetails(records, text):
    ...
    # Extract the Invoice items along with the Quantity and Rate.
    # These items exist in the text in the pattern of a word (item name) followed by 3 numbers.
...
# Special Note: 
# I have not implemented Exception Handling because the parameters of layout generalization
# have not been defined for the problem statement.mentType
